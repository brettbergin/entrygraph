    ...
    def matcher(x: FileExtraction) -> list[EntrypointHint]:
        ...
        for symbol, decorator in _decorated(x):
            ...
            if route is not None:
                hints.append(
                    EntrypointHint(
                        rule_id=f"python.{framework}.route",
                        kind=EntrypointKind.HTTP_ROUTE,
                        handler_qualified_name=symbol.qualified_name,
                        route=route,
            ...
            if pattern.match(decorator):
                hints.append(
                    EntrypointHint(
                        ...
                        framework=framework,
                        route=first_string_arg(decorator)
                        if kind is EntrypointKind.HTTP_ROUTE
                        else None,
...
def _aiohttp_routes(x: FileExtraction) -> list[EntrypointHint]:
    ...
    for ref in x.references:
        if ref.kind == "call" and ref.callee_name in _AIOHTTP_ADD and ref.arg_preview:
            ...
            hints.append(
                EntrypointHint(
                    rule_id="python.aiohttp.route",
                    kind=EntrypointKind.HTTP_ROUTE,
                    handler_qualified_name=None,
                    route=route or "",
                    http_methods=[ref.callee_name.replace("add_", "").upper()],
                    framework="aiohttp",
                    metadata={"registration": ref.arg_preview},
                )
            )
...
register(
    EntrypointRule(
        "python.flask.route", "python", "flask", EntrypointKind.HTTP_ROUTE, _flask_routes
    )
)
register(
    EntrypointRule(
...
register(
    EntrypointRule(
        "python.flask.middleware", "python", "flask", EntrypointKind.MIDDLEWARE, _middleware
    )
)
register(
    EntrypointRule(
        "python.fastapi.middleware", "python", "fastapi", EntrypointKind.MIDDLEWARE, _middleware
    )
)
register(
    EntrypointRule(
        "python.fastapi.route", "python", "fastapi", EntrypointKind.HTTP_ROUTE, _fastapi_routes
    )
)
register(
    EntrypointRule(
        "python.django.urls", "python", "django", EntrypointKind.HTTP_ROUTE, _django_urls
    )
)
register(
    EntrypointRule(
        "python.click.command", "python", "click", EntrypointKind.CLI_COMMAND, _click_commands
    )
)
register(
    EntrypointRule(
...
register(
    EntrypointRule("python.celery.task", "python", "celery", EntrypointKind.TASK, _celery_tasks)
)
register(
    EntrypointRule(
...
register(
    EntrypointRule(
        ...
        "bottle",
        EntrypointKind.HTTP_ROUTE,
        _sanic_bottle_routes("bottle"),
    )
)
register(
    EntrypointRule(
        "python.aiohttp.route", "python", "aiohttp", EntrypointKind.HTTP_ROUTE, _aiohttp_routes
    )
)
register(
    EntrypointRule(
        ...
        "pyramid",
        EntrypointKind.HTTP_ROUTE,
        _decorator_rule(_VIEW_CONFIG, "python.pyramid.view", EntrypointKind.HTTP_ROUTE, "pyramid"),
    )
)
register(
    EntrypointRule(
        "python.dramatiq.actor",
        "python",
        "dramatiq",
        EntrypointKind.TASK,
        _decorator_rule(_DRAMATIQ_ACTOR, "python.dramatiq.actor", EntrypointKind.TASK, "dramatiq"),
    )
)
register(
    EntrypointRule(
        ...
        "airflow",
        EntrypointKind.TASK,
        _decorator_rule(_AIRFLOW_TASK, "python.airflow.task", EntrypointKind.TASK, "airflow"),
    )
)
register(EntrypointRule("python.core.main", "python", None, EntrypointKind.MAIN, _main_guard))
