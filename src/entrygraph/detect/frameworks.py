...
FRAMEWORKS: list[FrameworkSpec] = [
    ...
    FrameworkSpec("argparse", "python", (FrameworkSignal("import", "argparse", 0.7),)),
    FrameworkSpec(
        "celery",
        "python",
        (
    ...
    FrameworkSpec("net/http", "go", (FrameworkSignal("import", "net/http", 0.7),)),
    FrameworkSpec(
        "spring-boot",
        "java",
        (
    ...
    FrameworkSpec("rake", "ruby", (FrameworkSignal("file_presence", "Rakefile", 0.6),)),
...
def detect_frameworks(
    ...
    for spec in FRAMEWORKS:
        ...
        for signal in spec.signals:
            evidence = _check_signal(
                signal, spec, manifests, import_signals, file_paths, symbol_names
            )
            if evidence:
