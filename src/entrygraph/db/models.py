...
# ---------------- Continuous Reachability Gate (#116) ----------------
#
# The gate stores its baselines/findings/policy alongside the graph in the same
# (global) database, keyed by repo_id. All FKs cascade on repo delete so an
# uninstalled/purged repo takes its gate state with it.


class Baseline(Base):
    """The accepted set of reachable dangerous paths for a repo's default branch.

    A new PR is gated by diffing its paths against the newest baseline; the
    baseline is refreshed from the protected branch after merge. One row per
    (repo, branch); its accepted paths live in ``BaselinePath``."""
    ...
    commit_sha: Mapped[str | None] = mapped_column(String(64))  # commit it was cut from
    created_at: Mapped[datetime]
    path_count: Mapped[int] = mapped_column(Integer, default=0)

    __table_args__ = (UniqueConstraint("repo_id", "branch", name="uq_baseline_repo_branch"),)


class BaselinePath(Base):
    """One accepted path in a baseline, addressed by its stable fingerprints."""

    __tablename__ = "baseline_paths"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    baseline_id: Mapped[int] = mapped_column(ForeignKey("baselines.id", ondelete="CASCADE"))
    fingerprint: Mapped[str] = mapped_column(String(32))  # strict blake2b-128 hex
    endpoint_fingerprint: Mapped[str] = mapped_column(String(32))
    source_category: Mapped[str | None] = mapped_column(String(48))
    sink_id: Mapped[str | None] = mapped_column(String(64))
    risk: Mapped[float] = mapped_column(Float, default=0.0)
    path_json: Mapped[str | None] = mapped_column(Text)  # rendered hops for reports

    __table_args__ = (
        Index("ix_baseline_paths_fp", "baseline_id", "fingerprint"),
        Index("ix_baseline_paths_endpoint", "baseline_id", "endpoint_fingerprint"),
    )


class ScanRun(Base):
    """One gate evaluation of a head commit against a baseline."""
    ...
    status: Mapped[str] = mapped_column(String(16))  # passed|failed|warned|error
    ...
    __table_args__ = (Index("ix_scan_runs_repo", "repo_id", "created_at"),)


class Finding(Base):
    """A single path surfaced by a scan run, classified against the baseline."""

    __tablename__ = "findings"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    scan_run_id: Mapped[int] = mapped_column(ForeignKey("scan_runs.id", ondelete="CASCADE"))
    fingerprint: Mapped[str] = mapped_column(String(32))  # strict
    endpoint_fingerprint: Mapped[str] = mapped_column(String(32))
    source_category: Mapped[str | None] = mapped_column(String(48))
    sink_id: Mapped[str | None] = mapped_column(String(64))
    risk: Mapped[float] = mapped_column(Float, default=0.0)
    ...
    __table_args__ = (Index("ix_findings_scan", "scan_run_id", "status"),)


class Suppression(Base):
    """A reviewed waiver: a fingerprint the gate must not fail on."""
    ...
    __table_args__ = (Index("ix_suppressions_repo_fp", "repo_id", "fingerprint"),)


class RepoPolicy(Base):
    """Per-repo gate policy. Absent row -> built-in defaults (see gate.store)."""
    ...
    risk_threshold: Mapped[float] = mapped_column(Float, default=0.5)  # gate at/above this
    gated_categories: Mapped[str | None] = mapped_column(Text)  # JSON list; null = all
