      ...
      <div className="row wrap" style={{ marginBottom: 12 }}>
        ...
        {path.verified === true && (
          <span className="row" style={{ gap: 2 }}>
            <Label size="small" variant="success">taint verified</Label>
            <InfoPopover term="taint_verified" />
          </span>
        )}
        {path.verified === false && (
          <Label size="small" variant="secondary">no flow proven</Label>
        )}
        {path.edges.some((e) => e.sanitized_by.length > 0) && (
          <span className="row" style={{ gap: 2 }}>
            <Label size="small" variant="attention">sanitizer seen</Label>
            <InfoPopover term="sanitizer" />
          </span>
        )}
